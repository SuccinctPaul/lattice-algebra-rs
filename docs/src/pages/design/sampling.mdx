...
flowchart TD
    ...
    S --> G["DiscreteGaussian(σ)<br/>CDT + fast Fourier sampling<br/>(planned — Falcon milestone)"]
```
...
- The Gaussian sampler (CDT base + fast Fourier sampling) is a Falcon-milestone item and will be feature-gated with its non-CT status documented.

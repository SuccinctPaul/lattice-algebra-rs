...
Ascending coefficients, Horner evaluation, division with remainder (requires `R: Field` — division is a field operation), derivative, scalar product, and an FFT-based fallback multiplication (an in-tree radix-2 `f64` kernel) for non-2-adic moduli. SNARK-grade extensions (batch inversion, interpolation) are scheduled.
...
flowchart TD
    ...
    TAD -- no --> FFT["in-tree radix-2 f64 FFT<br/>O(n log n)"]
...
`Z_q[X]/(X^n − 1)` (for some SNARK parameters) is designed as a separate type sharing the L1 engine without the twist — type-level isolation prevents negacyclic/cyclic mixing. Falcon shipped as a self-contained reference port and does not require it.

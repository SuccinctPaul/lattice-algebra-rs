...
| `lattice-pqc` (`pqc`) | `crates/pqc` | `mldsa`, `mlkem` (L6 PQC, incl. the FIPS-exact NTTs `mldsa::ntt` / `mlkem::ntt`) + `falcon` (round-3 reference port) |
...
| Module | Owns | Status |
| --- | --- | --- |
| `mldsa` | ML-DSA-44/65/87 (FIPS 204): parameter ZSTs, `SigningKey`/`VerifyingKey`, deterministic & hedged signing, hint machinery, FIPS encodings; per-set API modules (`mldsa::mldsa_65::…`) | ✅ (126 ACVP vectors) |
...
| KAT framework (FIPS vectors, ACVP) | `tests/data/` + per-scheme harnesses (`acvp_kat`, `acvp_mlkem_kat`, `falcon_kat`) | ✅ (ML-DSA / ML-KEM ACVP + Falcon round-3) |

...
> One unified algebra foundation from which **NIST PQC schemes** (ML-DSA, ML-KEM and Falcon today; FN-DSA tracks the FIPS 206 draft) and **lattice-based zkSNARKs** (LaBRADOR / GreyHound / LatticeFold-style) derive directly — shipped as a three-crate workspace: [`algebra`](https://github.com/SuccinctPaul/lattice-algebra-rs/tree/main/crates/algebra) · [`pqc`](https://github.com/SuccinctPaul/lattice-algebra-rs/tree/main/crates/pqc) · [`zk`](https://github.com/SuccinctPaul/lattice-algebra-rs/tree/main/crates/zk).
...
- **L6 PQC**: **ML-DSA, ML-KEM and Falcon implemented and tested** — ML-DSA-44/65/87 (FIPS 204 encodings, deterministic + hedged signing, internal-`mu` and preHash interfaces, 279 ACVP vectors), ML-KEM-512/768/1024 (183 ACVP vectors), Falcon-512/1024 (round-3 spec, official KATs byte-exact). FN-DSA (FIPS 206) is still a draft: parameter sets may shift before the freeze — **no stable release until then**.
...
| Evidence | Artifact |
| --- | --- |
| 542 tests, ring axioms over 15 moduli (2 … 2³²) | `cargo test --workspace` |

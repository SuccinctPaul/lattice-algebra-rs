...
| Evaluation hiding / ZK openings (Jindo) | ✅ **closed 2026-09-25** (was: Fig. 7's two response gates not implementable on the proof object). The capability landed in `pcs::mle`: `MleLevels` (Fig. 3 `Com*` steps 3–4's two compressed levels — `t̂ₖ = ⌊tₖ/B_t⌉`, `û = ⌊(D t̂)/B_u⌉`), `MleQuadProof` (Fig. 7's *sent* response `t̂`, `f̂* := F̂*c`, `r := Rc`; lines 5–6's `h`, `s` stay verifier-side, as printed), `MleLevels::quad_report` returning the **whole** failing set of `QuadCheck::{InnerNorm, OuterNorm}`, and `QuadParams` deriving `B`, `B_o` through Theorem 3 → 5 → 6 from the instance. `examples/jindo.rs` now sends those messages and asserts both lines, each **failing alone** (measured: `long inner response f̂* → {L7}`, `stack shifted by q → {L8}`), with the honest transcript at `|f*|+|r|+|h| = 18811 ≤ B = 698424` and `|s|+|t̂| = 5309 ≤ B_o = 100863`. **Two transcription corrections, both against the PDF's glyph geometry and the paper's own arithmetic**: (a) lines 7–8 gate a **sum of ℓ2 norms**, not of squares — the `2` after `‖f̂*‖`, `‖h‖`, `‖t̂‖` is the norm subscript (it sits at the subscript baseline; B.6 p. 18, B.8 p. 19 and B.9 p. 20 all *add roots* to reach `B = b√((m₁+1)d) + B_χ√((μ+ν)d) + B_t√(μd)`), so the gates of the earlier audit note and of this row are mis-written; (b) Theorem 5's `σ = 14/(ln M)·m₀√n₀·B_C·B` is a **product** (Lemma 2 p. 6: `σ ≈ 14T/ln M`), i.e. a larger `B` needs a *larger* σ — which the toy fails (**σ required = 689 136** at the input bound `B = 4102`, sampled `σ = 336`, now printed rather than hidden; this row said `2 753 520` before, which is stale — the printed line is the authority). Residual, documented in the example header: `B = [B′|I_μ]`'s identity block is absent here (Thm. 2's MLWE hiding still not delivered), the value-mask row is exempt from `L7` for the same `p = q` reason `NORM` exempts it, and `NORM`'s tamper now trips `{NORM, L7}` together because both read the sub-polynomial rows. Evidence: `open-milestones.md` §"Jindo 的 Fig. 7 L7/L8" and its follow-up section | `pcs::masking`, `pcs::mle` |
| Ring lookup arguments | ⏳ research branch, entry [2026/471](https://eprint.iacr.org/2026/471) | survey ⏳ item 23 |

### 4.1 Per-scheme assembly status (all 14 §1 schemes, one line each)

§1 is the objective's checklist, so this is the audit view: every scheme, the
example that assembles it, and **where that example states its own limits** — the
heading is named rather than restated, so the claim cannot drift from the file.
Evidence line: `scripts/integration_gate.sh`, whose step 2 prints one row per
scheme. Measured on this tree (2026-09-25 ~20:04): **14 PASS / 0 FAIL**, `--lib`
**665/0**, `protocol_contract` **14/0**, all four feature lanes OK,
`cargo test --workspace` **1352/0 across 19 binaries**, 0 compile-error lines,
gate exit 0.

| §1 scheme | example | what it states about itself |
|---|---|---|
| Greyhound | `examples/greyhound_pcs.rs` | every mode drives the same traits; the core is welded to the Z1 instance — generalising it is task #18, whose measured surface (173 fixed-const references, 9 NTT references, 9 importers) is recorded there |
| LaBRADOR | `examples/labrador.rs` | "What the recursion costs, and whether it pays here": one composed level, and the level *costs* bytes at this shape |
| SLAP | `examples/slap.rs` | "Instance — a structural demonstration, not a security level" |
| FMN / PowerBASIS | `examples/fmn.rs` | same heading; its norm-gate-independence case (the `ker A` vector) is the row §4 names |
| Rinocchio | `examples/rinocchio.rs` | "What this scheme is *not*, and the page says so explicitly": not ring-SIS, no Fiat–Shamir, designated verifier, trusted linear-size setup, QRP not QAP |
| CMNW | `examples/cmnw.rs` | §4's assembly row; under FS a false `u` trips all seven equations at once, so the battery asserts whole failing sets (eq. (6) is the one isolable case) |
| CELPC | `examples/celpc.rs` | "Scope: the non-hiding ("w/o ZK") variant" |
| Orbweaver | `examples/orbweaver.rs` | "This scheme needs a trusted setup, and says so" — and the gate line is the finding: a trapdoor-forged opening of a false `y` verifies with every printed condition holding |
| Serval | `examples/serval.rs` | "Instance, and where it departs from the paper": `log q = 12` against Table 4's 88–128, and no LaBRADOR proof-of-proof compaction |
| Hachi | `examples/hachi.rs` | §4's assembly row: p. 7's substituted claim `Σ P(i)·Q(i) = V` is now *proved* over `F_{q^k}`; the recursion that would make its output succinct is not |
| Maltese | `examples/maltese.rs` | §4's assembly row + §5's `Π^Fin` note: Eq. (32)–(38) verified, `s_reshape` opened, so not succinct |
| Akita | `examples/akita.rs` | "What is not here, and why": the recursion, a security level, sublinear openings for §9's offload |
| Grand Danois | `examples/grand_danois.rs` | "Steps deliberately NOT taken": the recursion; eq. (19) now closes on its circuit |
| Jindo | `examples/jindo.rs` | "Steps covered"; `σ = 336` against the required `689 136` is printed, not hidden |
